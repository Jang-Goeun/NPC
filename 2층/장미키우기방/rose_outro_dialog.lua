...
function scene:create( event )
	...
	local font_Speaker = native.newFont("font/PF스타더스트 Bold.ttf") -- 원하는 폰트 파일 경로로 수정
	local font_Content = native.newFont("font/PF스타더스트.ttf") -- 원하는 폰트 파일 경로로 수정
	
	---$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ 
	local image_rose = display.newImage("image/오브제/rose0_glass.png")
	image_rose.x, image_rose.y = display.contentWidth*0.2, display.contentHeight*0.5
	image_rose.name = "장미"
	image_rose.alpha = 0
	...
	local speaker = display.newText(dialog, "파이", display.contentWidth*0.25, display.contentHeight*0.76, display.contentWidth*0.5, display.contentHeight*0.1, font_Speaker)
	speaker:setFillColor(0)
	speaker.size = 50
	--speaker.font = "font/PF스타더스트 Bold.ttf"
	
	local content = display.newText(dialog, "또 뭐해야하지..?", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content)
	...
	-- json에서 정보 읽기
	local Data = jsonParse("2층/장미키우기방/json/rose_pi2.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then --뷰 끝나면 다음씬으로!
			local function goToNextScene()
                ...
                composer.gotoScene("2층.장미키우기방.rose_ending", options)
            end
			goToNextScene()
			return
		end
		...
		elseif(Data[index].speaker == image_pi.name) then
			image_rose.alpha = 0
			image_pi.alpha = 1
		elseif (Data[index].speaker == " ") then
			image_rose.alpha = 0
			image_pi.alpha = 0
		end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene("2층.장미키우기방.rose_intro_dialog")
	elseif phase == "did" then
