...
local sound_rose = audio.loadSound("sound/장미키우기.mp3")
...
function scene:create( event )
	...
	local function gotoGame(event)
		if finger.alpha == 1 then
			composer.gotoScene("2층.장미키우기방.rose_intro_dialog", { effect = "fade", time = 500 })
			-- else if (isDoor == true) then
			-- 	--composer.removeScene("temp.lua")
			-- 	--composer.gotoScene("2층로비", { effect = "fade", time = 500 })
		end
	end
	--커서누르면 현재좌표출력
