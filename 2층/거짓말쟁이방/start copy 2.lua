...
-- ↓ 시작화면 배치 -----------------------------------------------------------------------------------------------
	...
	local interact_extent = display.newRect(display.contentCenterX, display.contentCenterY, 250, 250)
	interact_extent.x, interact_extent.y = display.contentWidth*0.5, display.contentHeight*0.375   -- game으로 넘어갈 수 있는 부분
...
-- ↓ 플레이어 이동 함수 정리 -------------------------------------------------------------------------------------------------
	...
	local function moveCharacter(event)
		if movingDirection == "up" then
			...
			if playerGroup.y > room.contentHeight - playerGroup.contentHeight * 7.4 then
		...
		elseif movingDirection == "left" then
			...
			if playerGroup.x > room.contentWidth - playerGroup.contentWidth * 14.62 then
