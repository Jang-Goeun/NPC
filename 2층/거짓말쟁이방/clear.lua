...
-- ↓ 시작화면 배치 -----------------------=----------------------------------------------------------------------
	...
	local key = display.newImage("image/거짓말쟁이방/key.png")
...
-- ↑ 시작화면 배치 ---------------------------------------------------------------------------------------
-- ↓ json ----------------------------------------------------------------------------------
	local Data = jsonParse( "2층/거짓말쟁이방/json/게임승리.json" )
...
-- ↓ key획득 -----------------------------------------------------------------------------------
	local function keytap( event )
		...
		composer.gotoScene("2층.거짓말쟁이방.clear2") -- clear2으로 넘어가기
	end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene('2층.거짓말쟁이방.clear') -- 추가
	elseif phase == "did" then
