...
-- ↓ 시작화면 배치 -----------------------=----------------------------------------------------------------------
	...
	local truth_teller = composer.getVariable( "truth_teller1")
...
-- ↑ 시작화면 배치 ---------------------------------------------------------------------------------------
-------------------------json1에서 정보 읽고 적용↓(임시)-----------------------------------------------------------
	local Data = jsonParse( "2층/거짓말쟁이방/round1_json/사람2.json" )
	...
	local function nextScript( event )
		...
		if wrong == 0 and c == -1 then
			if index == 3 then
				...
				composer.gotoScene("2층.거짓말쟁이방.gameround1")
				return
			end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene('2층.거짓말쟁이방.gameround1_사람2') -- 추가
	elseif phase == "did" then
