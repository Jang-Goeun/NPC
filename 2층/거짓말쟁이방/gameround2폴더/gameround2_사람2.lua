...
-- ↓ 시작화면 배치 -----------------------=----------------------------------------------------------------------
	...
	local truth_teller = composer.getVariable( "truth_teller2")
	...
	character.x, character.y = display.contentWidth*0.37, display.contentHeight*0.5   -- 주인공
...
-- ↑ 시작화면 배치 ---------------------------------------------------------------------------------------
-------------------------json1에서 정보 읽고 적용↓(임시)-----------------------------------------------------------
	local Data = jsonParse( "2층/거짓말쟁이방/round2_json/사람2.json" )
	...
	local function nextScript( event )
		...
		if wrong == 0 and c == -1 then
			if index == 3 then
				...
				composer.gotoScene("2층.거짓말쟁이방.gameround2")
				return
			end
		...
		if i == 1 then    -- 진술 
			...
			if c == 1 and doublecheck == 1 then
				...
				composer.gotoScene("2층.거짓말쟁이방.gameround2_black")
				return
			elseif wrong == 1 then
					wrong = 0
					i = 0
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene('2층.거짓말쟁이방.gameround2_사람2') -- 추가
	elseif phase == "did" then
