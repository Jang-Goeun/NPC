...
function scene:create( event )
	...
	local function tap( event )
		if index == 1 then
			composer.gotoScene("2층.2층백신.백신2")
			Vaccine.y = Vaccine.y - 50
		elseif  index == 2 then
			composer.gotoScene("2층.2층백신.백신3")
			Vaccine.y = Vaccine.y - 100
		elseif  index == 3 then
			composer.gotoScene("2층.2층백신.백신4")
			Vaccine.y = Vaccine.y - 150
		elseif  index == 4 then
			audio.play(gunSound)
			composer.gotoScene("2층.2층백신.백신5")
		end
	end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		--composer.removeScene( "2층.2층로비_ending2" )
	elseif phase == "did" then
