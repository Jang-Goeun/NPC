...
--[[
    ...
    fram[2] = display.newImageRect("image/artist/오브제 액자.png", fram[1].width, fram[1].height)
    fram[2].x, fram[2].y = 950, 180
    fram[3] = display.newImageRect("image/artist/오브제 액자.png", fram[1].width, fram[1].height)
    fram[3].x, fram[3].y =1200, 180
    fram[4] = display.newImageRect("image/artist/오브제 액자.png", fram[1].width, fram[1].height)
...
local smallerPic = nil  -- 빈원형 위에 생성될 작은 사진


-- Function to create a smaller version of the pic on not_interaction
local function createSmallerPic(event)
    ...
    smallerPic = display.newImageRect(target.fillPath, 100, 50)
    smallerPic.x, smallerPic.y = not_interaction.x, not_interaction.y
    sceneGroup:insert(smallerPic)
    
    -- Add tap event listener to the smallerPic to restore the original pic
    smallerPic:addEventListener("tap", function(event)
        smallerPic:removeSelf()
        smallerPic = nil  -- Reset the reference to smallerPic
    end)
end
...
local function createSmallerPic(event)
    ...
    -- Create a smaller version of the pic on top of not_interaction
    smallerPic = display.newImageRect(target.fillPath, 200, 200)
    smallerPic.x, smallerPic.y = not_interaction.x, not_interaction.y
    sceneGroup:insert(smallerPic)
...
-- Add tap event listeners to each pic
for i = 1, 4 do
    pic[1].fillPath = "image/artist/예술가의방 그림 H.png"
    pic[2].fillPath = "image/artist/예술가의방 그림 U.png"
    pic[3].fillPath = "image/artist/예술가의방 그림 N.png"
    pic[4].fillPath = "image/artist/예술가의방 그림 T.png"
    pic[i]:addEventListener("tap", createSmallerPic)
end
]]
