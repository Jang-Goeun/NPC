...
function scene:create( event )
	...
	local content = display.newText(dialog, "됐다! 내가 이겼어!", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content, 45)
	...
	-- json에서 정보 읽기
	local Data = jsonParse("2nd_floor_Vaccine_json/Vaccine6.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			composer.gotoScene( "2nd_floor_ending4" ) 
			return
		end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "2nd_floor_Vaccine6" )
	elseif phase == "did" then
