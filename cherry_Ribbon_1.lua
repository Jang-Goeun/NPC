...
function scene:create( event )
	...
	-- json에서 정보 읽기
	local Data = jsonParse("1st_floor_json/cherry_Ribbon_1.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			composer.hideOverlay("cherry_Ribbon_1")
			composer.gotoScene( "endingVideo" ) 
			return
		end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "cherry_Ribbon_1" )
	elseif phase == "did" then
