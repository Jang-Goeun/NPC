...
function scene:create( event )
	...
	local content = display.newText(dialog, "나한테 오는 건가? 오, 오지 마!", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content, 45)
	...
	-- json에서 정보 읽기
	local Data = jsonParse("2nd_floor_Vaccine_json/Vaccine3.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			composer.setVariable("index", 3)
			--composer.hideOverlay( "2nd_floor_Vaccine3" )
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "2nd_floor_Vaccine3" )
	elseif phase == "did" then
