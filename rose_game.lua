...
local function createBug()
    ...
    if side == "left" then
        ...
        bugL.alpha = 1
        bugL.x, bugL.y = display.contentWidth*0.3, display.contentHeight/2
        ...
        bugR.alpha = 1 
        bugR.x, bugR.y = display.contentWidth*0.7, display.contentHeight/2
    ...
    --터치 이벤트부여
    bugR:addEventListener("touch", onBugTouch) --터치하면 제거됨
    bugL:addEventListener("touch", onBugTouch) --터치하면 제거됨
...
local function onPotTouch(event) --드래그해서 장미에 놓으면, 물완료,장미정상 이벤트
    ...
    elseif ( event.phase == "ended" or event.phase == "cancelled") then
        if ( event.target.isFocus ) then
            ...
            if ( event.target.x > display.contentCenterX - 200 and event.target.x < display.contentCenterX + 200
 					and event.target.y > display.contentCenterY - 200 and event.target.y < display.contentCenterY + 200
...
local function changeRoseImage() --5초마다 소환
    ...
    if (roseGroup.x == bugL.x and bugL.alpha == 1) or (roseGroup.x == bugR.x and bugR.alpha == 1) then 
...
function scene:create( event )
	...
	memo.x, memo.y = display.contentWidth/2+50, display.contentHeight*0.12
    --파이
    ...
    local function checkGameOver() --1초마다호출해서 확인하기
        if (rose[3].alpha == 1) then --죽으면,
            gameStarted = false
            isDied = true
            composer.removeScene("rose_game")
            local function goToNextScene()
                local options = {
                    effect = "fade", 
                    time = 2000 
    ...
    local function isGameStarted(event)
        if gameStarted and not isDied then
            ...
            roseTimer = timer.performWithDelay(4000, changeRoseImage, 0) --4초마다 시들기
            bugTimer = timer.performWithDelay(9000, createBug, 0) --9초마다 벌레생성
            checkTimer = timer.performWithDelay(7000, checkSheep, 0) --7초마다 양 확인
