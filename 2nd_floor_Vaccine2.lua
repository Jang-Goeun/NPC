...
function scene:create( event )
	...
	local content = display.newText(dialog, "저기 있잖아...! 근데 왜 하필 계단을 막고 있는 거야?!", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content, 45)
	...
	-- json에서 정보 읽기
	local Data = jsonParse("2nd_floor_Vaccine_json/Vaccine2.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			composer.setVariable("index", 2)
			--composer.hideOverlay( "2nd_floor_Vaccine2" )
			composer.gotoScene( "2nd_floor_ending2" ) 
			return
		end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "2nd_floor_Vaccine2" )
	elseif phase == "did" then
