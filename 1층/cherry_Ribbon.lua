...
function scene:create( event )
	...
	local choice = {}
	choice[1] = display.newImageRect("image/UI/대답박스 분리.png", 900, 220)
	...
	-- json에서 정보 읽기
	local Data = jsonParse("1층/json/cherry_Ribbon.json")
	...
	local answer1 = display.newText("리본을 건넨다.", display.contentWidth * 0.76, display.contentHeight * 0.57, font_Content, 36)
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			composer.hideOverlay("1층.cherry_Ribbon")
			if cherry_interaction == 1 then
				composer.gotoScene( "1층.cherry_Ribbon_1" ) 
			elseif cherry_interaction == 0 then
				composer.gotoScene( "1층.cherry_Ribbon_2" ) 
			end
		else
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "1층.cherry_distractor" )
	elseif phase == "did" then
