...
function scene:create( event )
	...
	-- json에서 정보 읽기
	local Data = jsonParse("1층/json/cherry_Ribbon_2.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			composer.hideOverlay("1층.cherry_Ribbon_2")
			composer.gotoScene( "1층.ending" ) 
			return
		end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "1층.cherry_Ribbon_2" )
	elseif phase == "did" then
