...
function scene:create( event )
	...
	cherry.x, cherry.y = display.contentWidth/2, 350
	...
	frame[1].x, frame[1].y =  display.contentWidth * 0.22, -600
	frame[2].x, frame[2].y =  display.contentWidth * 0.25, display.contentHeight * 0.1
        ...
        elseif movingDirection == "down" then ---------------------
			...
			if ( background.y > 202 and 434 > background.y or 934 < background.y and playerGroup.y < 998 ) then 
				if ( playerGroup.y < -280 ) then
					playerGroup.y = playerGroup.y + moveSpeed
				elseif ( playerGroup.y > -280 and background.y > 82) then -- 여기 숫자 각 맵에 맞게 조절하시면 됩니다. ex) -608
					moveCamera(moveSpeed)
				elseif (playerGroup.y < 204) then
					playerGroup.y = playerGroup.y + moveSpeed
				end
			elseif ( playerGroup.x >= -132 and playerGroup.x <= 132) then
				if ( playerGroup.y < -280 ) then
					playerGroup.y = playerGroup.y + moveSpeed
				elseif ( playerGroup.y > -280 and background.y > 82) then -- 여기 숫자 각 맵에 맞게 조절하시면 됩니다. ex) -608
					moveCamera(moveSpeed)
