...
function scene:create( event )
	...
	local content = display.newText(dialog, "아직 못 얻은 아이템이 있어", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content, 45)
	...
	-- json에서 정보 읽기
	local Data = jsonParse("2nd_floor_json/2nd_floor_lobby_X.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			composer.hideOverlay( "2nd_floor_lobby_X" )
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "2nd_floor_lobby_X" )
	elseif phase == "did" then
