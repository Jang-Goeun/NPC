...
function scene:create( event )
	...
	local image = display.newImage("image/캐릭터/파이 안도.png")
	...
	local content = display.newText("휴.. 살았다. 끝난건가?", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content , 45)
	...
	local function nextScript( event )
		if isInteraction[6] == false then
			interactionNumber = interactionNumber + 1
			isInteraction[6] = true
		end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "배경화면.security_winEnding" )
	elseif phase == "did" then
