...
function scene:create( event )
	...
	-- json에서 정보 읽기
	local Data = jsonParse("배경화면/json/security_script.json")
	...
	local content = display.newText(dialog, "bksjndlasndlsndksd", display.contentWidth*0.5, display.contentHeight*0.88, display.contentWidth*0.7, display.contentHeight*0.2, font_Content)
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			-- 값 넘기기
			composer.setVariable("programNumber", 6)
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "배경화면.security_script" )
	elseif phase == "did" then
