...
function scene:create( event )
	...
	local first = composer.getVariable("loginFirst")
	...
	if first == nil then
		composer.gotoScene("배경화면.mainScript3")
	end
	...
	function onNextButtonTap()
	    ...
	    if passWord == "등잔밑이어둡다" then
	        ...
	        composer.removeScene('배경화면.login_logIn') 
	        -- audio.stop() --로그인 성공하면 loading으로 이동하면서 bgm 정지
	        audio.play(gameSuccess)
			composer.gotoScene("배경화면.robot_mail")
	    else
	        print("틀렸습니다. 다시 입력해주세요.")
			composer.showOverlay("배경화면.login_script", {isModal = true})
	...
	local function passWord( event )
        ...
        display.remove(inputPassword)
		composer.removeScene('배경화면.login_logIn') 
		composer.gotoScene('배경화면.login_passwordFind')
	end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.setVariable("loginFirst", 1)
		composer.removeScene('배경화면.login_logIn') -- 추가
