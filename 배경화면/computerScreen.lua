...
function scene:create( event )
	...
	if interactionNumber == 3 then 
		...
		composer.gotoScene("배경화면.mainScript2")
	...
	local function transfer( event )
		...
		if programNumber == 1 then -- 게임
			if(internetEnd ~= 1 ) then
				composer.gotoScene("배경화면.game_Nstart_loading") --인터넷 에피 끝나기 전
			else
				composer.setVariable("internetEnd", false)
				composer.gotoScene("1층.game_start_loading") --인터넷 에피 끝난 후 
			end

		elseif programNumber == 2 then -- 과제 
			composer.gotoScene("배경화면.screen_project")
		elseif programNumber == 3 then -- 일기
			composer.gotoScene("배경화면.screen_diary")
		elseif programNumber == 4 then -- 사진
			composer.gotoScene("배경화면.screen_picture")
		elseif programNumber == 5 then -- sms
			composer.gotoScene("배경화면.screen_kakao")
		...
		elseif programNumber == 7 then -- 내pc
			composer.gotoScene("배경화면.screen_myPC")
		elseif programNumber == 8 and internetEnd ~= 1 then --인터넷 에피 끝나기 전 인터넷

			if underbar.alpha == 1 then -- 와이파이 온
				if interactionNumber < 4 then 
					composer.gotoScene("배경화면.screen_internet")
				else
					composer.gotoScene("배경화면.login_logIn")
				end
			else -- 와이파이 오프
				composer.gotoScene("배경화면.dino_intro")
			end
		elseif programNumber == 8 and internetEnd == 1 then --인터넷 끝난 후 못들감
			if underbar.alpha == 1 then -- 와이파이 온
				composer.gotoScene("배경화면.screen_noTransfer")
			else
				composer.gotoScene("배경화면.dino_intro")
			end

		elseif programNumber == 9 then -- 휴지통
			composer.gotoScene("배경화면.screen_garbageScript")
