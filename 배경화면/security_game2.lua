...
function scene:create( event )
	...
	for i = 1, 10 do
		security[i] = display.newRect(display.contentCenterX, display.contentCenterY, 400, 200)
		security[i]:setFillColor(0, 0, 1)
		security[i].name = i

		securityGroup[i]:insert(security[i])

		button[i] = display.newRect(display.contentCenterX, display.contentCenterY, 80, 50)
	...
	security[8].x, security[8].y = display.contentWidth * 0.6, display.contentHeight * 0.15
	...
	local function cancel( event )
		...
		if(cancelNum == 10) then
			print("게임성공~~!!!!!")
			composer.gotoScene( "배경화면.winEnding" )
		end
	...
	for i = 1, 10 do
 	...
 	timer.performWithDelay(220, counter, 51)
