...
function scene:create( event )
	...
	local program9_new = display.newImage("image/휴지통/휴지통 노랑.png") --노랑 휴지통 
	program9_new.x, program9_new.y = 1040, 673 
	program9_new:scale(0.36, 0.36)
	sceneGroup:insert(program9_new)

	local function trashcan_blink( image )
	    transition.to(image, { time = 300, alpha = 0, onComplete = function()
	        transition.to(image, { time = 300, alpha = 1, onComplete = function()
	            trashcan_blink(image) 
	        end })
	    end })
	end

	trashcan_blink(program9_new)
	...
	local function transfer( event )
		...
		if programNumber == 1 then
			...
			composer.gotoScene( "compass_script2" )
			--composer.gotoScene("배경화면.cherryFirstMeet") --임시
	...
	-- ↓ 휴지통 입장 수정 코드  --------------------------------------------------------------------------------------------------
			 	...
			 	composer.gotoScene( "compass_holeScript" )
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "compass_trashcan" )
