...
function scene:create( event )
	...
	local content = display.newText(dialog, "어떻게 하지? 이렇게 죽을 순 없어!", display.contentWidth*0.5, display.contentHeight*0.902, display.contentWidth*0.7, display.contentHeight*0.2, font_Content, 45)
	...
	-- json에서 정보 읽기
	local Data = jsonParse("2nd_floor_Vaccine_json/Vaccine4.json")
	...
	local function nextScript( event )
		...
		if(index > #Data) then 
			composer.setVariable("index", 4)
			--composer.hideOverlay( "2nd_floor_Vaccine4" )
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene( "2nd_floor_Vaccine4" )
	elseif phase == "did" then
